...
|----|--------|--------|------------|
| P0-1 | DFA scale limitation disclosure; reframe lock-in language | **RESOLVED** | §5.5 discloses 40 turns, <1 log-decade scale range. Lock-in language systematically qualified throughout: abstract, §1, §4.1, §5.1, §5.3, §5.6, §6 all use "elevated persistence" or "consistent with" framing. §5.3 heading retained ("The Lock-in Problem") as a conceptual category, but body text now qualifies with DFA limitations cross-reference. §5.6 reframed from "indicate rigid repetition structure" to "consistent with rigid repetition structure... though confirming this interpretation requires longer sessions and stationarity testing." Figure captions retain "consistent with lock-in" (qualified form). The α > 1.0/1.2 boundary is not a coded threshold (code uses 0.70–0.90 for healthy range) — paper now treats it as directional, not definitive. |
